 ...
 *
 * 値は環境変数 EBEX_MODEL_PROFILES（JSON 文字列）で slug 単位に部分上書きできる。
 * 例: EBEX_MODEL_PROFILES={"gpt-5-nano":{"continuations":{"nudgeMax":15}}}
 */
...
function readOverrides(): Record<string, PartialProfile> {
  ...
  try {
    ...
    if (!warnedInvalidOverrides) {
      ...
      console.warn(
        "[model-profiles] EBEX_MODEL_PROFILES の JSON が不正なため無視します",
      );
    }
  }
