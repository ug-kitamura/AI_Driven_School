...
AI 応答に対して **「エディタに上書き」** アクションを提供しなければならない（SHALL）。操作は応答テキストから **markdown コードブロック**（```markdown ... ``` または ``` ... ```）を抽出し、存在すればその**内側の本文**（開始行の ` ```markdown ` / ` ``` ` および対応する閉じフェンス行を除く）を、なければ応答全文を選択中レッスンの編集内容で **置換** しなければならない（SHALL）。抽出結果にフェンス行自体を含めてはならない（MUST NOT）。本文中にネストしたコードブロック（例: ` ```bash `）がある場合、外枠 markdown ブロックの閉じフェンスまで欠落なく抽出しなければならない（SHALL）。4 バッククォート外枠（` ````markdown ` で始まる形式）で包まれた応答でも、内側の ` ```markdown ` ブロック本文を正しく抽出しなければならない（SHALL）。上書き前に確認ダイアログを表示しなければならない（SHALL）。上書き成功後、Pane 3 を **編集ビュー** に自動切替しなければならない（SHALL）。「エディタに上書き」と「コピー」は AI 応答のストリーミングが完了した後にのみ表示されなければならない（SHALL）。
