...
**次のセッションの入口は「ラウンド10 の実機確認の聞き取り」**。曼陀羅の問題 5 件を change 2 本で直し、archive とコミット・プッシュまで済ませた（2026-08-21・→ 1章）。⚠ **ただし「中心が合ったか」はブラウザペインでは原理的に判定できない**ので、**実機で見てもらうまで直ったとは言えない**（→ 7章の盲点）。確認してほしい項目は 1章に列挙してある。
...
```
[次] ラウンド10 の実機確認                     ← 次セッションの入口。⚠ 最優先
       曼陀羅の 5 件を change 2 本で直した（2026-08-21・push 済み）。
       ⚠ **ペインで見えない領域があるので、実機で見てもらうまで
          「直った」と言えない**（→ 7章。ラウンド9 はここで穴を通した）。
       聞くこと:
         ① ペイン2 のミニ曼陀羅サムネイルに絵が出るか  ← 直った確証あり※
         ② 全体曼陀羅モーダルを開いた直後、グラフが中心に来るか ← ⚠ 未確認
         ③ モーダルでパンしたあとウィンドウを変えても戻らないか  ← ⚠ 未確認
         ④ ライトの曼陀羅がサイトと同じに見えるか      ← 実測では一致※
         ⑤ カードの大きさ（260×96 → 230×88）は適当か
         ⑥ ミニ曼陀羅の Start / Goal が意図どおりか
         ⑦ ゴーストの実線で別シリーズだと分かるか
            （とくに compact のサムネイル。手がかりが opacity .85 だけになる）
         ⑧ ピンクテーマの地がニュートラルになったのが物足りなくないか
       ※ ①④はペインで数値まで取れているので、期待とズレたら
          「実装が違う」ではなく「意図の取り違え」を先に疑う

       触る前に読む: `openspec/changes/archive/2026-08-21-mandala-sizing-and-fit/`
       と `.../2026-08-21-mandala-appearance-polish/` の design.md
...
- ⚠ **ブラウザペインでは React Flow の辺が描画されない。** ノードの実測（ResizeObserver / rAF）が完了するまでノードは `visibility: hidden` で辺も描かれず、**ペイン非表示だと計測が完了しない**。辺が0件・fitView 未適用（`scale(1)` のまま）に見えても実装の異常ではない。同様に **Base UI の Select はポップアップが座標を持たず操作できない**。機構はメモリ `project-browser-pane-verification-limits`
  ...
  - ⚠ **ペインのビューポートは 0×0 のことも実寸（1280×720 等）のこともある。まず `innerWidth` を読むこと**（2026-08-21）。0×0 でも `getComputedStyle().height` や `offsetWidth` は解決するので高さの伝播・ノード寸法・色は測れるが、**`max-h-[90vh]` と `min-h-[20rem]` の区別がつかない**など「ビューポート由来の長さ」を含む判定は全部あてにならない。実寸が返るときは `resize_window` で低いビューポートも試せる
