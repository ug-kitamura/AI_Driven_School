...
GlobalHeader に **DXトレーニング曼陀羅** と **設定（歯車）** がある。設定では Anthropic API キー、テーマ（ライト／ダーク／システム）、ペイン既定幅を変更できる。
...
- **AI**: 本文スロットを Claude + Playwright で PNG 生成 → `images/ai/`（staging）→ 挿入で promote（要 API キー）
- **Web**: 準備中（次フェーズ）
