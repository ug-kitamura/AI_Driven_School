...
ユーザーが Agent チャットに未送信の入力（本文テキストおよびファイル添付チップ）を持ったまま別プロジェクトフォルダを選択し、元のプロジェクトフォルダに戻ったとき、システムは戻り先の未送信入力（本文と添付チップ）を復元しなければならない（SHALL）。下書きはプロジェクトフォルダ（`scopeId`）単位でメモリ上に保持しなければならない（SHALL）。メッセージ送信が完了したとき、当該プロジェクトの下書きを消去しなければならない（SHALL）。この下書き保持はメモリ上のみとし、ページのリロードやアプリ再起動をまたいで永続化してはならない（MUST NOT）。既にプロジェクト単位で永続化されているスキル選択ラベル（`activeSkillId`）の挙動は変更してはならない（MUST NOT）。
