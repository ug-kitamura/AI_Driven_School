...
2026-08-10〜13 に、EBEX agent の移植・作業ファイル置き場の再設計・`workspace/` 廃止・`contents/` 書込ゲート・スキル改善4本（プレビュー / create / plan 命名 / コンテキスト梯子）・名前確認ゲート・run レイアウト調整（`reviews/` と曼陀羅の図）・Git シリーズ6本の原稿更新パスまでが完了している。

### リポジトリの状態

ブランチ **`dx-training-studio3`**。2026-08-13 の作業は **`981b561` までで概ねコミット済み**（名前確認ゲート → Git シリーズのリネームと原稿更新 → ハンドオフ統合）。

**未コミットで残っているのは run レイアウト調整の5ファイル**（`SKILL.md` / `contracts/agent-write-contract.md` / `openspec/specs/{contents-plan-layout,training-create-skill}/spec.md` / 本文書）。ステージ済みだがコミットはされていない。

⚠ これらのコミットは**こちらから `git commit` を叩いていない**。6章の「勝手にコミットが生まれる」現象が今日も起きた（あるいは別ツールでの手作業）。**作業の区切りで `git log` を見る運用を続けること。**
