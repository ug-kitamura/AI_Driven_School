 ...
 * prompts.ts（プロンプト）にもある——3者は translation-freshness spec の
 * 固定順（root=[name,description] / series=[名,catch,desc] /
 * course=[名,catch,desc,target] / lesson=[名,desc]）で一致していること。
 */
...
import type { MetaSourceFields } from "@/lib/translation/freshness";
...
/** 階層ごとの `_en` フィールド名（author_en は鮮度・翻訳の対象外なので含めない） */
export const UNIT_EN_KEYS: Record<UnitLevel, string[]> = {
