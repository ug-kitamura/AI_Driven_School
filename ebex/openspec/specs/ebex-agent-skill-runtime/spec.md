...
### Requirement: スキルの起動は明示指定のみ

システムはスキルをユーザーの明示指定によってのみ起動しなければならない（SHALL）。明示指定とは Agent 入力の `/` オートコンプリートによる選択、または同等の UI 操作を指す。ユーザーのメッセージ本文、スキルの `description`、スキル名等のトリガーワードとの一致からスキルを自動的に選択・起動してはならない（MUST NOT）。スキル実行中に、実行中スキルの本文の指示を根拠として別スキルを起動してはならない（MUST NOT）。

#### Scenario: トリガーワードでは起動しない

- **WHEN** ユーザーがスキルを選択せずに、あるスキルの `description` に含まれる語（例「議事録を作って」）だけを送信する
- **THEN** スキルは起動せず、通常のチャットとして処理される

#### Scenario: 明示指定で起動する

- **WHEN** ユーザーが `/` で候補からスキルを選択して送信する
- **THEN** 選択されたスキルが起動する

#### Scenario: スキルから別スキルへ連鎖しない

- **WHEN** 実行中スキルの本文に別スキルの起動を促す記述がある
- **THEN** システムは別スキルを起動せず、実行中スキルの処理を続行する
...
#### Scenario: 中間ファイルは作業フォルダ直下の \_work/ を案内する

- **WHEN** スキル実行のランタイム文脈が生成される
- **THEN** 文脈は「中間ファイルは作業フォルダ直下の `_work/` に置く」旨を含み、成果物フォルダの位置に依存させず、特定のフォルダ名（`output/` 等）を成果物の置き場として固定しない

#### Scenario: 成果物が output/ にあっても中間ファイルの位置は変わらない

- **WHEN** スキルが成果物の置き場としてプロジェクト内の `output/` を明示している状態でランタイム文脈が生成される
- **THEN** 文脈が案内する中間ファイルの置き場は `output/_work/` ではなくプロジェクトフォルダ直下の `_work/` である
...
システムはスキル本文に文字列「サブエージェント」が含まれる場合、EBEX が真のサブエージェントを起動できないことをユーザーに明示し、当該役割を独立した文脈で実行する `run_isolated_task` ツールによる代替へフォールバックしなければならない（SHALL）。v1 の検出キーワードは「サブエージェント」のみとし、英語表記や Task tool 名の広範検出は必須としない（MUST NOT require）。フォールバック時、ランタイムはモデル向けに「サブエージェントは起動できないが、`run_isolated_task` ツールで当該役割を独立した文脈で実行できる」旨の短い案内を添えてよい（MAY）。`run_isolated_task` は親の会話履歴を引き継がない子 LLM 呼び出しに限定し、子が自律的にツールを呼び出す真のサブエージェントを実装してはならない（MUST NOT）。

#### Scenario: キーワード検出時にユーザーへ案内する

- **WHEN** 本文に「サブエージェント」を含むスキルをユーザーが invoke する
- **THEN** ユーザーに対し、EBEX は真のサブエージェントには非対応であり、独立実行ツールで代替する旨が表示される
...
#### Scenario: 検出後も既存の agent loop で実行を続ける

- **WHEN** 「サブエージェント」を含むスキルが検出される
- **THEN** システムは実行を中止せず、既存の単一 agent loop でスキル実行を続行し、当該役割は `run_isolated_task` の呼び出しとして実行されうる

### Requirement: 額縁差し込みの抽象注意（場の説明）

スキル実行時のランタイム場の説明は、額縁テンプレートへの差し込みを案内する場合、次を特定の記法やスキル名に依存せず短く添えてよい（MAY）：(1) 手順メモにはその媒体のコメント構文を用い、埋める印にはコメント構文を使わない (2) 可変長の差し込み区間には明示の開始・終了トークンを用いる。テンプレ方言のカタログやエンジン別例外を場の説明に列挙してはならない（MUST NOT）。当該注意は推奨であり、ランタイムがモデルのツール呼び出しを改変してはならない（MUST NOT）。

#### Scenario: 場の説明に抽象注意を含めてよい

- **WHEN** スキル実行のランタイム文脈が生成され、額縁の copy→replace 案内が含まれる
- **THEN** 文脈は記法カタログ無しで、コメントと埋める印の分離および明示区間端についての短い注意を含んでよい

#### Scenario: 注意はツール実行に介入しない
