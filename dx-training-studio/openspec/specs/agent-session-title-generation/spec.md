...
- **WHEN** プレースホルダーまたは LLM 生成タイトルが 30 文字を超える内容に基づく
- **THEN** 永続化される `title` の末尾に `…` が付かない
