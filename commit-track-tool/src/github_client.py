...
公開メソッド一覧:
	...
	get_reviews(pr_numbers, since, until)      PR ごとの期間内レビュー数とレビュアー別実績を返す
...
class GitHubClient:
	...
	def get_reviews(
		...
		"""
		期間内のレビュー実績を集計する。
		戻り値: (レビュアー別 activity, PR 番号ごとの期間内レビュー件数)
		"""
		activity: dict[str, dict] = {}
		per_pr: dict[int, int] = {}
		for i, number in enumerate(pr_numbers, 1):
			...
			for review in reviews:
				...
				if not (since <= submitted_at <= until):
					continue
				per_pr[number] = per_pr.get(number, 0) + 1
