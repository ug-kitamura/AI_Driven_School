...
**次のセッションの入口は「改善ラウンド10 の課題出し」**。ラウンド9（2026-08-21）まで片付いており、**次の課題はまだ出ていない**。⚠ **憶測で着手せず、まず課題を聞くこと**——このプロジェクトは毎回ユーザーが実機を触って課題を出し、explore →（必要なら）propose → apply → archive で回している。
...
```
[次] 改善ラウンド10 の課題出し                      ← 次セッションの入口
       ラウンド9 までは片付いている。**次の課題は未共有**。
       ⚠ **まず課題を聞くところから始める**（憶測で着手しない）。
...
[次] ラウンド9 の PR → マージ                       ⚠ 人がやる
       ...
       ラウンド9 の change（dx 側3本・EBEX 側2本）:
         ・studio-pink-theme          Studio にピンクテーマ
         ・studio-editor-search-polish 折りたたみホバー拡大＋検索ハイライト
         ・studio-mandala-react-flow  曼陀羅を Mermaid → React Flow
         ・ebex/pink-theme            EBEX にピンクテーマ（ハート置換つき）
         ・ebex/editor-search-polish  EBEX へ同じ 2 点を横展開
       ⚠ **人の目視で最終判断する項目**（機構はコード・DOM で検証済み）:
         ・ピンクの実際の色味（桜色の面 / ショッキングピンクの塗り）
           トークン値と AA コントラストは実測済みだが、**印象は人が見る**
         ・**曼陀羅の辺**（Studio・サイトとも）
           ⚠ **ブラウザペインでは描画されない**（→ 7章）。
           対照実験でサイトの既存曼陀羅も同じく 0 辺になることを確認済みなので
           移植の不具合ではないが、**辺が出ること自体は実機で見ていない**
         ・**折りたたみ ▼ が行番号列のホバーで出るか**（両アプリ）
           ⚠ 実ホバーは検証できていない。CSS ルールの存在・詳細度
           （show は `:hover` の分だけ hide より高い）・DOM 構造は確認済み
         ・ミニ曼陀羅の拡大モーダルの幅が分岐 3 つ以上で足りるか
