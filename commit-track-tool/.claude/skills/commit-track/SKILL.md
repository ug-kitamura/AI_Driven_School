...
| `{{ comitora_logo_data_uri }}` | なし（置換しないこと） |
...
| `{{ feedback_in_progress }}` | `pr.summary.feedback_in_progress_count` |
...
- Mermaid `gitGraph` の **DSL 本体のみ** を出力する。
- `<style>` や `<script>`、HTML 要素は **一切出力しない**。
- 1行目に init ディレクティブで配色を固定し、続いて `gitGraph` ブロックを出力する。
- 時系列順に古い PR から並べる。
- ブランチ名はそのまま使う（`/` は使えないので含む場合は `_` に置換する）。
- `commit id:` はそのブランチの主要コミットメッセージを 1〜3 単語に要約。
- `tag:` には PR 番号とマージ日（MM/DD）を入れる。
- **重要** 最終更新が直近のブランチから最大8個にしぼる（default ブランチ含めると最大で9個）
...
- 全てのアクションのカードの背景色とボーダー色を統一する。
- 各アクションのカード内に「誰が」と「いつまで」をバッジで明示する（blueまたはgray）
...
- **PDF印刷で消える表現を使わない** — グラデーション文字はPDF出力時に透明のまま消える。グラデーション文字を使った場合はPDF出力で色が単色に変わる点を意識すること
- **`{{ comitora_logo_data_uri }}` を変更しない** — 後の工程でPythonスクリプトが画像データを埋め込むのに使用するため、必ず元の形のまま残す
