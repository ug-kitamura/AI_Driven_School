...
Pane4 で取得した画像は `dx-training-editor/images/{source}/` に **staging として** 保存しなければならない（SHALL）。`source` は `uploaded`・`ai`・`web` のいずれかである。UP タブからの取得は `uploaded` を用いなければならない（SHALL）。`images/` 配下（正本・staging・trash）は `.gitignore` の `/images/*` により git 追跡対象外としなければならない（SHALL）。
