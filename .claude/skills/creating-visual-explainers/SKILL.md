...
- `references/base.html` — 図解テンプレート（Tailwind CSS CDN・Lucide Icons CDN・カスタム配色を含む「額縁」）
...
> テンプレートファイルが見つかりません。スキルのフォルダ構成が壊れている可能性があります。
...
模範回答のデザインを参考にしつつ、Tailwindの語彙で自由にレイアウトを組む。
模範回答のパターンに合うものはそのまま使い、合わないものはTailwindクラスでその場で作る。
テンプレートの定義済みパーツに縛られない。
