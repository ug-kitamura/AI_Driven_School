...
シリーズ・コース階層の `_meta` に「概要」という独立項目を出してはならない（SHALL NOT）。概要ページはフォルダ自身のページとして生成し（index に `asIndexPage: true`）、サイドバーのシリーズ名・コース名のクリックでトグル開閉と同時に概要ページが開かなければならない（SHALL）。ルート `_meta` の「トップ」項目は残す。
