...
**次のセッションの入口は「細かい課題」を聞く**（ユーザー宣言・2026-08-22）。⚠ **中身は意図的に未記録**——「次のセッションでやる／そのとき聞く」とユーザーが決めたので、**書き忘れではない**。**セッション冒頭で内容を聞くところから始めること**。実装を急がず explore で分解してから change に落とす。器は整っているので、聞いてから優先順を決めればよい。

**課題が軽ければ先に「英訳パイロット」**（→ 1.5 節）。**翻訳まわりの UX と器は 2026-08-22 に全て片付いた**ので、パイロットはいつでも走れる。

**英語翻訳対応（4大課題④）**は 2026-08-21 に器（change 3本）、**2026-08-22 に UX ラウンド（3本）と機能ラウンド（7本）**で作り込んだ。すべて archive 済み。⚠ **2026-08-22 の7本で Studio の翻訳 UI は作法ごと変わった**（言語がアプリのモードになり、言語切替の置き場・操作ボタンの配置・検索の対象が全部動いた）。**1.5 節を読まずに触ると、撤回済みの規則で実装してしまう**。⚠ **曼陀羅の表示崩れは立ち上げ直しで解消したため対象外**（ユーザー判断）。

積み残しの実機確認が1件ある: **曼陀羅モーダルを閉じて開き直したとき中心が合うか**（→ 1章。ペインでは原理的に判定できない）。**ユーザーに聞くだけ。**

⚠ **`dx-english-translation` はコミット済みだが未 push**（origin より4コミット先行・2026-08-22 時点）。前回この行は「push 済み」だったので、**そのつもりで PR を探しに行かないこと**。コミットはユーザーが自分でまとめたもので、AI 側からのコミットは相変わらずしない（→ 6.3）。
...
```
[次] 「細かい課題」を聞く                       ← 次セッションの入口
       ⚠ 中身は**意図的に未記録**。2026-08-22 の機能ラウンド（change 7本）の
       あとに出たもので、「次のセッションでやる」とユーザーが決めた。
       書き忘れではないので、どこかに書いてないか探さないこと。
       セッション冒頭で「何が気になっているか」を聞き、explore で分解する
...
[次] 英訳パイロット（英語翻訳対応④の仕上げ）    ← 上が軽ければ先にこちら
       ...
       実装済みの器と作法は 1.5 節
       ✅ 残量は**機械で出せる**ようになった（引数なしで走査すると全体の
          進捗表が出る）:
            node --experimental-strip-types \
              .claude/skills/dx-training-translate/references/scan-freshness.mts
          2026-08-22 時点は 本文 1/9・メタ 2/17・changelog 最新

[次] dx-english-translation ブランチの push → PR → マージ    ⚠ 人がやる
       英語翻訳対応の change 13本（器3・UX3・機能7）＋ spec 同期＋翻訳契約＋
       スキル一式。⚠ **コミット済みだが未 push（4コミット先行）**
