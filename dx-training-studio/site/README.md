...
- **supergraphic 帯の `z-index: 40` はテーマのナビバー（`z-30`）より上、という前提の数値**。ナビバーの `z-index` が上がると帯がその下に潜る。壊れ方は「帯が隠れる」で機能に影響しないが、クラス名ではなく数値の依存なので更新時に気づきにくい。⚠ **帯は `position: fixed` でフローから外している**——`sticky` に戻すと帯が本文フローの先頭 6px を占め、同じく `sticky top:0` のナビバーがスクロール開始直後に 6px ずり上がってから固定される（サイドバーと目次も追随する）
