...
1. **手順メモ（ガイド）** — その媒体のコメント構文を使う。置換対象にしない。区間端トークンの字面をガイドに書かない（同じ字面が二重にあると、差し込みツールが誤った「最初の組」を拾う）。
2. **短スロット** — 消える前提の明示トークン。字面はスキルが決めてよい（例: `{{NAME}}`）。コメント構文以外を使う。
3. **区間端（可変長の差し込み）** — 明示の開始・終了トークンで区間を示し、タグの偶然一致に頼らない。区間の中身は空でもよい。レンダリングされて人が読む成果物では、媒体で無害な構文を**推奨**する（HTML なら `<!-- NAME_START -->` / `<!-- NAME_END -->`）。エディタ上のスキル実行だけが目的なら必須ではない（ホスト強制もしない）。
