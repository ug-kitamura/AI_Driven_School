...
右側の4ボタンは、通常時に**引いたグレー**（サブテキスト色より一段濃い操作色）で表示しなければならない（SHALL）。ホバー時は4ボタンとも**前景色（`--foreground`）**へ変化しなければならない（SHALL）——ホバーで primary（青）へ変えてはならない（MUST NOT）。青は本文リンクの語彙として残し、ヘッダーの操作には使わない。通常色の実値は公開サイトのナビバー右上ボタンと同じ濃さに揃え、目視で確定する。
...
#### Scenario: 右上4ボタンはホバーで青くならない

- **WHEN** 曼陀羅・社内コンテキスト・GitHub・設定のそれぞれにマウスホバーする
