...
**次のセッションの入口は「ラウンド9 の曼陀羅で見つかった問題の聞き取り」**。ユーザーが実機で確認した結果、**曼陀羅に問題が複数あり、そのうち重大なものが含まれる**（2026-08-21）。⚠ **問題の中身はまだ共有されていない**ので、**まず聞くところから始める**（憶測で着手しない・→ 1章）。曼陀羅以外（ピンクテーマ・検索ハイライト・折りたたみホバー）は**実機で問題なしと確認済み**。

⚠ **ラウンド9 はユーザーの明示指示でコミット・プッシュまで済ませた**（ブランチ `dx-ebex-pink-search-mandala`）。**これは例外**で、既定は「コミットしない」のまま（→ 6.3）。PR とマージは人がやる。
...
```
[次] 曼陀羅の問題の聞き取り → 修正            ← 次セッションの入口。⚠ 最優先
       ラウンド9 の React Flow 化を**ユーザーが実機で確認したところ、
       曼陀羅に問題が複数見つかった。重大なものを含む**（2026-08-21）。
       ⚠ **問題の中身はこの文書に無い（まだ共有されていない）**。
       ⚠ **憶測で直しに行かない**——まず何が起きているかを聞く。
       流れは explore →（必要なら）propose → apply → archive

       ⚠ **「ブラウザペインで検証できなかった項目」と混同しないこと。**
       ラウンド9 では辺の描画・折りたたみのホバーをペインで見られず
       未確認のまま残したが（→ 7章）、**実機で問題だったのは曼陀羅**で、
       **折りたたみホバーとピンクテーマ・検索ハイライトは問題なしと確認済み**。
       つまり未確認だった項目のうち、残っているのは曼陀羅側だけ

       触る前に読む: `openspec/changes/archive/2026-08-20-studio-mandala-react-flow/`
       の design.md（移植の判断）と tasks.md（実装メモ・検証の限界）

[次] ラウンド9 の PR → マージ                       ⚠ 人がやる
       ブランチ `dx-ebex-pink-search-mandala`（push 済み）。
       このマシンに `gh` CLI が無いので PR はブラウザから作る
       ⚠ **曼陀羅の問題を直してからマージするか、先にマージして
          追随の change で直すかは人が決める**
       ラウンド9 の change（dx 側3本・EBEX 側2本）:
         ・studio-pink-theme          Studio にピンクテーマ        ✅ 実機OK
         ・studio-editor-search-polish 折りたたみホバー拡大＋検索ハイライト ✅ 実機OK
         ・studio-mandala-react-flow  曼陀羅を Mermaid → React Flow ⚠ 問題あり
...
ブランチは作業の区切りで変わる（⚠ **`git branch --show-current` を信じること**）。2026-08-20 時点は **`main`**（PR #152 マージ済み・その上に改善ラウンド8 の未コミット差分）。⚠ **コミットするならブランチを切ってから**。コミットの作法は 6.3、横から入るコミットの話は7章。
...
- ⚠ **`npm run format` はリポジトリ全体を書き換える**（Studio で 200 ファイル以上・2026-08-21 実測）。**コミット済みのコードが現行 prettier のバージョンで整形されていない**ため、走らせると自分の変更と無関係な折り返しの差分が大量に混ざる。tasks.md に「format する」と書いてあっても**そのまま実行しない**——触ったファイルだけに `npx prettier --write <files>` を当てるか、そもそも当てない（**変更前から prettier 未整形なら、新たな違反を増やしていない**）。全体整形をやるなら独立した change にする
