...
**次のセッションの入口は「改善ラウンド9 の課題出し」**。ラウンド8（2026-08-20）まで片付いており、**次の課題はまだ出ていない**。⚠ **憶測で着手せず、まず課題を聞くこと**——このプロジェクトは毎回ユーザーが実機を触って課題を出し、explore →（必要なら）propose → apply → archive で回している。

⚠ **作業ツリーに未コミットの差分がある**（改善ラウンド8・14ファイル）。実装から archive・spec 同期まで終わっているが、**コミットは意図的にしていない**（差分を学習材料にする運用。→ 6.3）。次の作業を始める前に `git status` と `git log` を見ること。
...
```
[次] 改善ラウンド9 の課題出し                       ← 次セッションの入口
       ラウンド8 までは片付いている。**次の課題は未共有**。
       ⚠ **まず課題を聞くところから始める**（憶測で着手しない）。
       流れは explore →（必要なら）propose → apply → archive

[次] 改善ラウンド8 の差分をコミット → PR → マージ    ⚠ 人がやる
       **未コミット・14ファイル**（下記2本。implement → archive・spec 同期まで完了）。
       ⚠ **ブランチは `main`**。コミットするならブランチを切ってから
       このマシンに `gh` CLI が無いので PR はブラウザから作る
       ラウンド8 の change:
         ・header-action-affordance  両アプリのヘッダー操作の色/ホバー/カーソル
                                     ＋サイトのキャッチ前スペース（8件）
         ・studio-breadcrumb-trail   Studio パンくず刷新（4件）
       ⚠ **人の目視で最終判断する値**（機構は実機 DOM で検証済み）:
         ・右上ボタンの通常色 ライト #4B5563（= サイトの --dxm-navbar-action）
         ・**ダークだけ両アプリで約 0.09 の明度差が残る**
           Studio `--header-action` = oklch(0.80) / サイト = #a3a3a3 ≒ oklch(0.72)。
           サイトの通常色は今回の指摘に無いので動かしていない。
           揃えるならサイトの --dxm-navbar-action（ダーク）も同じだけ上げる
         ・ホバーの色変化と左上ロゴの opacity-75 の効き具合
           ⚠ **ブラウザペインでは検証不能**（遷移が凍結する。→ 7章）
         ・パンくずのリンク段が「カーソルだけ変わる」で発見しづらくないか
           （押せることを色で示さないのは指定どおり）
...
- **ミニ曼陀羅は `flowchart TB`（縦向き）**。右列は幅より高さのあるセルなので、横向きだと幅に合わせて縮小されノード名が読めない
- ⚠ **右列セルの高さは左列4項目だけで決まる。** グラフの実寸を行の高さ計算に参加させると左列の行間が引き伸ばされるため、セルの中身は `absolute inset-0` でフローから外し、SVG は `height:100%` ＋ `preserveAspectRatio="meet"` で縮んで収まる（表示は小さくなるが仕様）。**寸法の正本は `scaleMiniMandalaThumbnailSvg`（TS）のインライン style** で、`globals.css` の `.mini-mandala-thumbnail svg` に `width`/`height` を書き戻さないこと——`!important` がインラインに勝って再発する（2026-08-19 に実際に踏んだ）
