...
英語ページの本文は同フォルダの `contents.en.md` を使い、**存在しない場合は日本語本文へフォールバックし、未翻訳であることを示すバッジを表示しなければならない**（SHALL）。`contents.en.md` が存在する場合は、その原文ハッシュ（`translation-freshness` capability）を現在の `contents.md` と突き合わせ、**古い（不一致または未記録）ならば「翻訳が古い」ことを示すバッジを表示しなければならない**（SHALL）。最新ならばバッジを表示しない（SHALL NOT）。`contents.en.md` の1行目の原文ハッシュコメントは、レンダリングされる本文に含めてはならない（SHALL NOT）。

表示テキスト（シリーズ名・コース名・description・catch・コースの受講対象者）は `.meta.json` の `_en` フィールド（受講対象者は `target_en`）を使い、無ければ日本語へフォールバックしなければならない（SHALL）。
...
- **THEN** 英語の本文が表示される
- **AND** 翻訳が古いことを示すバッジが表示される

#### Scenario: 最新の翻訳にはバッジが出ない

- **WHEN** `contents.en.md` の原文ハッシュが現在の `contents.md` と一致するレッスンの英語ページを開く
- **THEN** 英語の本文が表示され、未翻訳・翻訳が古いのどちらのバッジも表示されない
