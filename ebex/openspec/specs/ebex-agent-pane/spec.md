...
スキル実行開始時に「サブエージェント」キーワードが検出された場合、Agent Pane（チャットまたは同等の目立つ UI）に、EBEX が真のサブエージェントには非対応であることと、当該役割を独立した文脈で実行するツールで代替する旨を表示しなければならない（SHALL）。この表示は実行中止の確認ダイアログであってはならない（MUST NOT）。

#### Scenario: チャットまたはバナーに案内が出る

- **WHEN** 「サブエージェント」を含むスキルの実行が開始される
- **THEN** Agent Pane 上に、真のサブエージェントには非対応であり独立実行ツールで代替する旨の案内が表示される
...
クライアントのストリーム消費層（`consumeAgentStream` 相当）は、サーバが定義するすべての `ConfirmKind` の `confirm_required` イベントを確認UI（インライン表示またはモーダル）へ転送できなければならない（SHALL）。受理する kind の一覧はサーバ定義と単一の共有定数から導出しなければならず（SHALL）、リテラル列挙の重複によって乖離が生じる構造にしてはならない（MUST NOT）。サーバとクライアントの kind 一覧の一致は自動テストで検証されなければならない（SHALL）。クライアントが解釈できない未知の kind を受信した場合、イベントを黙って破棄してはならず（MUST NOT）、`POST /api/agent/tool-confirm` へ即時に拒否応答を送信してサーバ側の確認待ちを解放しなければならない（SHALL）。

#### Scenario: 全 ConfirmKind が確認UIへ到達する
...
- **THEN** すべての kind で `onConfirmRequired` が呼び出され、対応する確認UI表示が可能である
...
#### Scenario: 自動発火なしの行が表に含まれる

- **WHEN** ユーザーが空のチャット（メッセージ 0 件）を見る
- **THEN** 表に「スキルは自動で始まらない → `/` で選ぶ」旨の ✗ 行が含まれる
...
### Requirement: ツール確認は連続要求でも応答を受理する

システムは、成果物を連続生成するスキル等でツール確認要求が連続する場合でも、各確認をユーザーが確実に応答（許可・拒否）できるようにしなければならない（SHALL）。確認要求ごとに独立した表示状態を持ち、直前の確認の表示状態の残留によって次の確認の操作が阻害されてはならない（MUST NOT）。ターン実行中の確認要求はインライン表示のため、ポータル型モーダルの mount/unmount 競合に起因する状態残留（pointer-events 詰まり等）は構造的に生じない。表示コンポーネントは確認要求の識別子（`toolUseId`）を key として扱い、確認要求ごとに独立した項目として描画しなければならない（SHALL）。

#### Scenario: 連続する確認をすべて応答できる

- **WHEN** 1 回の実行で書き込み／生成の確認要求が連続して発生する
- **THEN** 各確認はチャット欄に順に追加表示され、ユーザーはそれぞれで許可／拒否を実行でき、応答がサーバへ届く

#### Scenario: 直前の確認の残留で操作不能にならない

- **WHEN** ある確認を承認した直後に次の確認要求が届く
- **THEN** 次の確認項目はクリーンな状態で表示され、ボタンのクリック操作が阻害されない

### Requirement: フォルダ切替による中断は保留中のツール確認を解決する

システムは、フォルダ切替に伴う中断が発生したとき、その時点で表示中（保留中の）ツール確認要求も同時に解決（拒否相当）しなければならない（SHALL）。保留中の確認が宙吊りのまま残ってはならない（MUST NOT）。

#### Scenario: 切替中断で保留中の確認が閉じる

- **WHEN** ツール確認が表示されている状態でフォルダ切替の中断が承認される
- **THEN** 保留中の確認は拒否相当で解決され、確認UIは消える

### Requirement: ターン実行中の確認要求はペイン3へインライン表示する

ターン実行中に発生する確認要求（`ConfirmKind` が `overwrite` / `run-script` / `run-skill-script` / `generate-write` / `inline-assets` / `web-search` / `web-search-manual` のいずれか）は、Agent ペイン（ペイン3）のチャット欄に、実行中のツール呼び出しログと同じ表示領域内でインライン表示しなければならない（SHALL）。ポータル型のモーダルダイアログで表示してはならない（MUST NOT）。インライン表示は、応答（承認・拒否、または `web-search-manual` の場合は結果貼付・スキップ）があるまでチャット欄に残り続けなければならない（SHALL）。ユーザーがチャット欄をスクロールしても、応答済みになるまでインライン表示は失われてはならない（MUST NOT）。

ターン開始前のユーザー単発操作に対するガード（レッスン本文上書き確認、画像・マルチモーダル非対応通知、プロジェクト外パス確認、出力先選択）は本要件の対象外とし、モーダル表示のままでよい（MAY）。

#### Scenario: 上書き確認がチャット欄にインライン表示される

- **WHEN** ターン実行中に `overwrite` 種別の確認要求が発生する
- **THEN** 確認は Agent ペインのチャット欄内に、承認・拒否ボタンを持つ項目としてインライン表示され、モーダルダイアログは開かない

#### Scenario: スクロールしても確認待ち項目は残る

- **WHEN** 確認待ちの項目がチャット欄に表示された状態でユーザーが会話を上にスクロールする
- **THEN** 未応答の確認待ち項目は消えず、下にスクロールすれば引き続き表示される

#### Scenario: ターン開始前のガードはモーダルのまま

- **WHEN** レッスン本文の上書き確認、画像非対応通知、プロジェクト外パス確認、または出力先選択が発生する
- **THEN** これらは従来どおりモーダルダイアログで表示される
