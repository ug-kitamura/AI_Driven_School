...
describe("rehypeSearchHighlight", () => {
  it("wraps matches in a mark element", () => {
    const root = tree(element("p", text("最初のTODO")));
    expect(run(root, "TODO")).toBe(
      "<p>最初の<mark.lesson-search-highlight>TODO</mark></p>",
    );
  });

  it("splits text around the match", () => {
    const root = tree(element("h2", text("TODOの話")));
    expect(run(root, "TODO")).toBe(
      "<h2><mark.lesson-search-highlight>TODO</mark>の話</h2>",
    );
  });

  it("highlights inside nested elements", () => {
    const root = tree(element("p", element("a", text("TODO"))));
    expect(run(root, "TODO")).toBe(
      "<p><a><mark.lesson-search-highlight>TODO</mark></a></p>",
    );
  });

  it("highlights every occurrence", () => {
    const root = tree(element("p", text("TODOとTODO")));
    expect(run(root, "TODO")).toBe(
      "<p><mark.lesson-search-highlight>TODO</mark>と" +
        "<mark.lesson-search-highlight>TODO</mark></p>",
    );
  });
  ...
  it("does not descend into marks it created", () => {
    ...
    const root = tree(element("p", text("TODO")));
    const once = run(root, "TODO");
    expect(run(root, "TODO")).toBe(once);
  });

  it("leaves the tree untouched for an empty query", () => {
    for (const query of ["", "   ", undefined]) {
      const root = tree(element("p", text("TODO")));
      expect(run(root, query)).toBe("<p>TODO</p>");
    }
  });
});
