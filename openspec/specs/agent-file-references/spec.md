...
- **THEN** `contents/**/*.md` の path と name のリストが返される

#### Scenario: 選択中レッスンを先頭にする
- **WHEN** `/api/agent/files?current=contents/foo/bar/lesson.md` を呼び出す
- **THEN** 応答の先頭が `contents/foo/bar/lesson.md` で、残りは path のアルファベット順である

### Requirement: 許可パスの制限
参照可能ファイルは `contents/` 配下の `.md` に限定されなければならない（SHALL）。それ以外のパスは一覧にも invoke 添付にも含めてはならない（SHALL NOT）。
