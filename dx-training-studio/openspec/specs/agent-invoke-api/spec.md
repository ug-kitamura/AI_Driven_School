...
`POST /api/agent/invoke` で `skillId: "create-draft"` を実行する際、クライアントは `variables` に `lessonMeta`（JSON 文字列）および `availableTags`（JSON 文字列）を含めなければならない（SHALL）。`lessonMeta` には選択中レッスンの現在値（`status`, `tags`, `description`, `estimated_minutes`, `author`）が含まれなければならない（SHALL）。
