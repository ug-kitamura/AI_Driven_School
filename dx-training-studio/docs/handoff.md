...
**次のセッションの入口は「改善ラウンド7 の課題出し」**。2026-08-19 の終了時点で**ユーザーが新たな課題を見つけているが、内容はまだ共有されていない**。⚠ **憶測で着手せず、まず課題を聞くこと。**

⚠ **作業ツリーに未コミットの差分がある**（改善ラウンド6・21ファイル）。実装から archive・spec 同期まで終わっているが、**コミットは意図的にしていない**（差分を学習材料にする運用。→ 6.3）。次の作業を始める前に `git status` と `git log` を見ること。
...
```
[次] 改善ラウンド7 の課題出し                       ← 次セッションの入口
       2026-08-19 のセッション終了時に**ユーザーが新たな課題を見つけている**が、
       内容は未共有。⚠ **まず課題を聞くところから始める**（憶測で着手しない）。
       前回と同じ流れなら explore → propose → apply → archive

[次] ブランチ `dx-studio-tree-parity` の PR 作成 → マージ   ⚠ 人がやる
       このマシンに `gh` CLI が無いのでブラウザから作る:
         https://github.com/ug-kitamura/AI_Driven_School/pull/new/dx-studio-tree-parity
       **このブランチには2ラウンド分が乗っている**:
         ・ラウンド5（EBEX パリティ3部作）= commit 50ed30d ほか。push 済み
         ・ラウンド6（下記3本）= **未コミット**。作業ツリーに残してある
       ラウンド6 の change（implement → archive・spec 同期まで完了）:
         ・tree-interaction-polish  ツリーの操作性7件
         ・pane2-meta-layout        コースメタ2列＋縦曼陀羅・画像セレクト順
         ・mandala-navbar-icons     Map→Network・アイコン寸法
       ⚠ **人の目視で最終判断する値**（機構は実機 DOM で検証済み）:
         ・ツリー背景の濃さ ライト4% / ダーク6.5%（薄すぎたら 5% / 7% へ）
         ・ミニ曼陀羅の可読性（小さいのは仕様。左列を伸ばさないため）
         ・ナビバー3アイコン ロゴ17.6 / GitHub16.0 / Network14.7px
           （小さすぎるなら Network を 17〜18 へ上げる）
         ・ドラッグ並べ替えの操作感 / 青レール・ガイド線の質感
           （ブラウザペインでは検証不能）
...
ブランチは作業の区切りで変わる（⚠ **`git branch --show-current` を信じること**）。2026-08-19 時点は `dx-studio-tree-parity`（origin と同期・その上に改善ラウンド6 の未コミット差分）。コミットの作法は 6.3、横から入るコミットの話は7章。
...
**Studio の既知の失敗**（これ以外は green。2026-08-19 時点で **928 passed / 6 skipped**（936 中）/ mandala は別途 **105 passed**）:
