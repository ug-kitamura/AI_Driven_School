...
describe("model-profiles", () => {
  afterEach(() => {
    delete process.env.EBEX_MODEL_PROFILES;
  ...
  it("applies EBEX_MODEL_PROFILES as a partial deep-merge override", () => {
    process.env.EBEX_MODEL_PROFILES = JSON.stringify({
      "gpt-5-nano": {
  ...
  it("ignores invalid EBEX_MODEL_PROFILES JSON and keeps defaults", () => {
    process.env.EBEX_MODEL_PROFILES = "{ broken";
    const nano = resolveModelProfile("gpt-5-nano");
