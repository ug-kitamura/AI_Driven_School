...
- フィードバックループ
- テンプレートパターン
- 額縁・配布物パターン（自己完結・コピー先行・assets・`_work/`）
- 条件分岐パターン
- アンチパターン
- サイズ目安
...
def generate_report(data, format="markdown", include_charts=True): # データ処理 # 指定形式で出力 # オプションでチャートを含める
```
...
HTML / CSS / SVG など「額縁テンプレート」を持つスキル向け。ホスト（EBEX・Claude Code 等）を問わず動く書き方にする。

### 配布物の自己完結

- CSS・フォント・小さな画像は**事前にインライン化**し、単一ファイル（または同梱セット）で完結させる
- ランタイムに「CSS を別途コピーしてリンクを書き換える」手順を依存させない

### コピー先行の一文（SKILL.md に明示）

額縁があるなら、手順の入口に次のような一文を置く:

```markdown
まず `references/base.html` を `output/{名前}.html` に `copy_file` でコピーし、
その後 `replace_in_file` / `replace_between` で断片を差し込む。
```

テンプレ全文を読んで再生成する経路を既定にしない。

### 額縁内の文字の役割（軽量モデル対応規約）

額縁の中の文字列は役割を混ぜない。**額縁への埋め込み操作は、モデルが挿入位置を判断せず機械的に実行できる形（プレースホルダー置換・区間差し込み）だけで完結させる**。軽量モデル（GPT-5 nano 等）はマーカーのない自由記述の指示から挿入位置を判断できないため、この規約が実行成功率を直接左右する。

1. **短スロット（変数）** — `{{XXX}}` 形式（大文字スネークケース）に統一する。`replace_in_file` の `replacements` map による一括置換だけで埋まる。残存はホストの完了ゲート（未置換検知）でも拾われる。
2. **区間端（可変長の差し込み）** — `<!-- XXX_START -->` / `<!-- XXX_END -->`（大文字スネークケース＋サフィックス）の組に統一する。`replace_between` だけで差し込める。区間の中身は空でもよい。
3. **手順メモ（ガイド）は額縁内に書かない** — 指示・注意書きは SKILL.md 側に書く。額縁内の自由記述コメントは、軽量モデルが指示として誤解釈する温床であり、マーカーとの混同も招く。**額縁内のコメントは `_START` / `_END` マーカーのみ**とする。区間端トークンの字面をガイド文に書かない（同じ字面が二重にあると、差し込みツールが誤った「最初の組」を拾う）。

「配布物の自己完結」（前述）とこの3項目により、額縁の操作は「copy_file → replacements 一括置換 → 区間差し込み」の決定的な3手に正規化される。参照実装: minutes-maid の `references/base.html`。

### `assets:` 宣言（任意・opt-in）

プロジェクトへコピーして使う材料を frontmatter で宣言できる（不明フィールドとして他ホストでは無視される）:
...
宣言があると、ホストがスキャン推定より優先して案内に使える。未宣言でも `references/` / `templates/` 直下の素材は推定されることがある。

### 中間ファイルは `_work/`

partial 等の中間ファイルは、**成果物と同じフォルダの `_work/`** に置く（例: `output/_work/partial.html`）。成果物本体と混在させない。特定の成果物フォルダ名（`output/` 等）を固定する必要はない。
