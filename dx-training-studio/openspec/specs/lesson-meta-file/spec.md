...
### Requirement: 既存レッスンの一発移行スクリプト

`studio/scripts/migrate-lesson-meta.ts` を実行すると、全レッスンの `contents.md` から frontmatter を除去し、その値からレッスン `.meta.json` を生成しなければならない（SHALL）。frontmatter の `series` / `course` / `lesson` キーは捨てなければならない（SHALL）。既存の `id` / `slug` は保持しなければならない（SHALL）。移行後の `contents.md` は本文のみで、本文の内容は移行前と同一でなければならない（SHALL）。frontmatter を持たない `contents.md` は変更してはならない（SHALL NOT）。

#### Scenario: frontmatter 付きレッスンを移行する

- **WHEN** frontmatter に `slug: three-areas`・`id: lsn-three-areas-unvulm`・`status: done` を持つ `contents.md` に対して移行スクリプトを実行する
- **THEN** レッスンフォルダに `.meta.json` が作成され、`slug` / `id` / `status` が引き継がれる
- **AND** `contents.md` は frontmatter が除去され本文のみになる

#### Scenario: 移行は冪等である

- **WHEN** 移行済み（frontmatter 無し）の `contents.md` に対して移行スクリプトを再実行する
- **THEN** `contents.md` と `.meta.json` は変更されない
