...
function buildFoldGutterMarkers(view: EditorView, enableFolding: boolean) {
  if (!enableFolding) return RangeSet.empty;
...
export function lessonFoldGutter(options?: { enableFolding?: boolean }) {
  ...
  const foldMarkerPlugin = ViewPlugin.fromClass(
    class {
      ...
      constructor(view: EditorView) {
        this.markers = buildFoldGutterMarkers(view, enableFolding);
      }
      ...
      update(update: ViewUpdate) {
        if (
          ...
          this.markers = buildFoldGutterMarkers(update.view, enableFolding);
        }
      }
    },
  );
