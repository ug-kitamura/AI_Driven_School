...
#### Scenario: 大きな成果物の使い分けを案内する

- **WHEN** スキル実行のランタイム文脈が生成される
- **THEN** 文脈は「データ由来は `run_script`、創作長文は `generate_and_write`」の使い分けを短く含み、本文を tool 引数に載せない旨を案内する
...
#### Scenario: HTML 専用の必須コピー案内を出さない

- **WHEN** 実行中スキルに `references/base.html` がある
- **THEN** ランタイム場の説明は「`.html` は必ずテンプレコピーせよ」のような HTML 専用強制手順を含まない
