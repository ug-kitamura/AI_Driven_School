...
<div>
  <div class="flex items-center justify-between mb-2">
    <span class="font-semibold">📌 マイルストーンタイトル</span>
...
### ブランチ状況マップ `{{ branch_rows }}`

`pr.open` と `branch.active_branches` を組み合わせて `<tr>` を繰り返す。
表示対象は **オープン PR があるブランチ** + **直近更新の stale ブランチ**（最大 8 件）。

**オープン PR が 0 件のフォールバック:**
`pr.open` が空の場合は `pr.merged` の直近 5 件を「最近マージされたブランチ」として表示する。
ステータスバッジは `<span class="badge badge-success">✅ Merged</span>` を使用し、`merged_at` を相対時刻（例: `2 days ago`）で表示する。
...
```html
<tr>
  <td class="font-mono text-xs" style="color:var(--blue-40);">ブランチ名</td>
  <td>
...
| `{{ hero_handle }}` | `@` + `aggregate.hero.login` |
| `{{ hero_prs_merged }}` | `aggregate.hero.prs_merged` |
| `{{ hero_reviews }}` | `aggregate.hero.reviews_submitted` |
| `{{ hero_achievement }}` | スコアが最も高い項目を一言で（例: `⚡ コミット最多`、`🔍 レビュー最多`） |
