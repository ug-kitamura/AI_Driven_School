...
### 2.2 次のセッションの仕事: 見つかった課題

サイトを実際に見て**複数の課題が見つかっている**（2026-08-14 時点で内容は未記録）。**次のセッションの最初の仕事は、課題の洗い出しと優先順位付け**。

> **⚠ ここに課題を書き足すこと。** 記録がないまま作業に入ると、同じ議論をやり直すことになる。

こちらで把握している弱点（ユーザー報告とは別）は 2.5 に列挙してある。
...
| 項目 | 状況 |
|---|---|
| **シリーズのカバー画像が仮** | Git基礎に検証用の `git-three-areas-flow.png` を流用しているだけ。はじめにシリーズは未設定。**Pane4 の UP モードで本番画像を用意して `.meta.json` の `cover` に設定する**（AI 生成は却下済み——チープさ・統一感・ブランド乖離） |
| **ホバートレースが見た目に効かない** | 現在のコンテンツは全コースが1本の鎖で繋がっており、どのノードから辿っても全ノードが経路に入るため減光が起きない。**実装は正しい**（純関数＋テストで担保）。枝分かれが増えると効く |
...
| 項目 | 状況 |
|---|---|
| **サイトの課題（ユーザー報告）** | **内容が未記録。次のセッションで最初に聞き取って 2.2 に書く** |
...
| **シリーズの cover 画像** | Git基礎は仮画像、はじめには未設定。Pane4 の UP モードで用意する（2.5） |
